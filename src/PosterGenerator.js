...
function PosterGenerator() {
  ...
  const [fillprimaryColor, setfillPrimaryColor] = useState();
  const [fillsecondaryColor, setfillSecondaryColor] = useState()
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            ...
            setPrimaryColor(response.data[2]);
            setSecondaryColor(response.data[3])
            setfillPrimaryColor(response.data[0])
            setfillSecondaryColor(response.data[1])
        } catch (error) {
            console.error(error);
        }
    };
    ...
    marginTop: "100px"}}>
      ...
      <div style={{ 'height': '100%', display: 'flex' }}>
        ...
        <div className='wrapper' style={{ backgroundColor: "#303030", 'height': '60%', 'width': '60%', 'maxHeight': '60%' }}>
          ...
          {genText && <p style={{ 'color': 'white', 'margin-left': '150px', 'margin-right': '150px', 'border-style': 'solid', 'border-color': 'grey', 'border-width': '1px', 'border-radius': '10px', 'margin-bottom': '10px', 'margin-top': '10px' }}>{genText}</p>}
        </div>
        <div style={{margin: "30px", borderRadius: "20px", width: "100px", height: "100px", backgroundColor: fillprimaryColor,}}>
        <p>{primaryColor}</p>
        </div>
        <div style={{margin: "30px", borderRadius: "20px", width: "100px", height: "100px", backgroundColor: fillsecondaryColor,}}>
        <p>{secondaryColor}</p>
        </div>
      </div> 
